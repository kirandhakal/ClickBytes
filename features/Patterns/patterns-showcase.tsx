  ...
  return (
    <>
      ...
      <section className="container py-12"><div className="grid gap-12">
        ...
        <div><h2 className="mb-5 text-2xl font-bold">Advanced utilities</h2><div className="grid gap-5 lg:grid-cols-2"><Card><CardHeader><CardTitle>Command, filters, language</CardTitle></CardHeader><CardContent className="grid gap-5"><Button variant="outline" onClick={() => setCommands(true)}><Command />Open command palette <kbd className="ml-auto text-xs text-muted-foreground">⌘K</kbd></Button><CommandPalette open={commands} onClose={() => setCommands(false)} onOpen={() => setCommands(true)} items={[{ label: "Go to dashboard", shortcut: "G D" }, { label: "Create project", shortcut: "C" }, { label: "Open settings" }]} /><FilterPanel options={["Active", "Draft", "Review"]} selected={filters} onChange={setFilters} /><LanguageSwitcher /></CardContent></Card><Card><CardHeader><CardTitle>Calendar, Kanban, infinite list</CardTitle></CardHeader><CardContent className="grid gap-6"><CalendarView /><KanbanBoard columns={[{ title: "Todo", items: ["Write brief"] }, { title: "Doing", items: ["Build UI"] }, { title: "Done", items: ["Set up repo"] }]} /><InfiniteList items={["First item", "Second item", "Third item"]} /></CardContent></Card></div></div>
